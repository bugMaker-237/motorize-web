namespace Motorize.Components
{
	...
	public partial class ChartValues
	{
		...
		private List<KeyValuePair<string, ChartValuesViewModel>> Datas;
		...
		public EventCallback LoadItem { get; set; }
		...
		private void Service_OnDataSaved(object sender, List<KeyValuePair<string, ChartValuesViewModel>> e)
		{
			if(e != null)
			this.Datas = e;
		}
		...
		private async void ShowModal(List<Tuple<decimal, decimal>> toDraw, Action<decimal> onModalClosed)
		{
			...
			string max = (toDraw.Select(i => i.Item1).Max() + 500).ToString();
			List<string> all = toDraw.Select(i => i.Item1.ToString()).ToList();
			all.Add(max);
		...
		void Submit()
		{
			bool res = this.Validations.ValidateAll();
			if (!res)
			{
				this.ExpandAll = true;
			}
